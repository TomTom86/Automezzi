...
func (this *AdminController) Manage() {
	...
	for x := range users {
		...
		rows += fmt.Sprintf("<tr><td>%d</td>"+
			"<td>%s</td><td>%s</td><td>%s</td><td>%s...</td><td><a href='http://%s/appadmin/update/%s!%s'>%s</a></td></tr>", 
			users[x].Id, users[x].First, users[x].Last, users[x].Email,	users[x].Reg_date.String()[:i], domainname, users[x].Email, parms,"modifica")
	}
	this.Data["Rows"] = template.HTML(rows)
